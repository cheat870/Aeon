...
_KEY_RE = re.compile(r"^\s*(?:BAKONG_KHQR_BASE|ABA_KHQR_BASE)\s*=")
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("Usage: python scripts/set_bakong_khqr_base_from_image.py <path-to-qr-image>")
        return 2
    ...
    lines = _set_env_value(lines, "BAKONG_KHQR_BASE", payload)
    _write_lines(env_path, lines)

    print(f"Saved BAKONG_KHQR_BASE to {env_path}")
    return 0
